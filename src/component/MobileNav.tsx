...
const MobileNav: React.FC<VisibleI> = ({isVisible, setIsVisible}) => {
  ...
  return (
    <div
      className={`fixed w-full inset-0 bg-black bg-opacity-50  duration-300 transition-transform z-40 flex-shrink-0 top-0 ${
        isVisible ?  "-translate-x-full absolute" : "fixed xl:sticky"
      }`}
      onClick={() => setIsVisible(!isVisible)}
    >
      <div
        className="fixed text-white flex flex-col  text-xl font-light font-roboto justify-between bg-gradient-to-r from-[#210a0a] via-[#271d1d] to-[#170808]  h-full w-[60vw] p-4 shadow-md top-0 left-0"
